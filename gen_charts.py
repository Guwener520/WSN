"""Generate charts from run_lite output data. Run separately if heatmap fails."""
...
print("Generating charts from re-run...")
...
for sc_name, d in all_data.items():
    ...
    # Coverage bars
    fig1 = go.Figure()
    sn = sorted(algo_names, key=lambda n: d["algorithms"][n]["mean_coverage"], reverse=True)
    for name in sn:
    ...
    env_hm = WSNEnvironment(**COMMON, **{k:v for k,v in sc.items() if k!='obstacles'},
                           obstacles=obs, seed=42)
    env_hm.set_positions(best_pos)
    cov = env_hm.coverage_matrix()

    # Reconstruct full-grid coverage counts (NaN for obstacle cells)
    full_cov = -np.ones(env_hm.n_total_grid_points)  # -1 = obstacle
    valid_idx = np.where(env_hm._valid_mask)[0]
    full_cov[valid_idx] = np.sum(cov, axis=0)
    cov_cnt_2d = full_cov.reshape(len(env_hm.grid_y), len(env_hm.grid_x[0]))
    cov_cnt_2d = np.where(cov_cnt_2d < 0, np.nan, cov_cnt_2d)
    ...
    if obs:
        for o in obs:
            if o["type"] == "rect":
                ...
                fig2.add_trace(go.Scatter(x=o["cx"]+o["r"]*np.cos(th), y=o["cy"]+o["r"]*np.sin(th),
                               fill="toself", fillcolor="gray", opacity=0.4, mode="none",
                               showlegend=False), row=1, col=1)
    fig2.add_trace(go.Heatmap(x=env_hm.grid_x[0], y=env_hm.grid_y[:,0], z=cov_cnt_2d,
